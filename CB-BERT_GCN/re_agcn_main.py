...
def load_rel(args):
    ...
    rel=['synonym','hypernym','uncorrelated', 'needA', 'needSpecific', 'correlated','hasA']
    rel_dict={}
    for r in  rel:
        tokens=['[CLS]']
        for word in r:
        ...
        input_ids += padding
        input_mask += padding
        rel_dict[r]={"text":input_ids,"mask":input_mask}
    return rel_dict
...
def train(args, model, tokenizer, processor, device, n_gpu, results={}):
    ...
    for epoch_num in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_iter):
            ...
            input_ids, input_mask, valid_ids, segment_ids, label_ids, dep_type_matrix = batch
            loss = model(input_ids, segment_ids, input_mask, label_ids,rel=rel,#e1_mask=e1_mask, e2_mask=e2_mask,
...
def evaluate(args, model, tokenizer, processor, device, mode="test", output_dir='./'):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        input_ids, input_mask, valid_ids, segment_ids, label_ids,  dep_type_matrix = batch
        with torch.no_grad():
            logits = model(input_ids, segment_ids, input_mask,rel=rel, #e1_mask=e1_mask, e2_mask=e2_mask,
