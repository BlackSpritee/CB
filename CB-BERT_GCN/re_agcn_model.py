...
class ReAgcn(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(config.hidden_size*2+7, config.num_labels)
    ...
    def forward(self, input_ids,  token_type_ids=None, attention_mask=None, labels=None,rel=None, # e1_mask=None, e2_mask=None,
        ...
        output = torch.cat([cls, sequence_output[:,0]], dim=-1)
        for name in rel:
            rel_ids = torch.tensor(rel[name]["text"], dtype=torch.long).unsqueeze(0).cuda()
            rel_mask = torch.tensor(rel[name]["mask"], dtype=torch.long).unsqueeze(0).cuda()
            rel_cls=self.bert(rel_ids,attention_mask=rel_mask, output_all_encoded_layers=False)[0][:,0]
            sim_feature=torch.nn.functional.cosine_similarity(cls,rel_cls).unsqueeze(-1)
            output=torch.cat([sim_feature,output],dim=-1)
